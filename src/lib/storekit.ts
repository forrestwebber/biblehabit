...
export type PurchaseOutcome =
  | { status: "success"; verified: true; tier: "pro" | "free" }
...
async function verify(signedTransactionInfo: string): Promise<{ ok: boolean; tier?: "pro" | "free"; error?: string }> {
  try {
  ...
  switch (result.status) {
    ...
    case "success": {
      ...
      if (verdict.ok) {
        ...
        return { status: "success", verified: true, tier: verdict.tier ?? "pro" };
      }
